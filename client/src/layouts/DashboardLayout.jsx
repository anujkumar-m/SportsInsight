...
const DashboardLayout = ({ children, title, subtitle }) => {
  ...
  return (
    <div className="min-h-screen min-h-dvh bg-background flex flex-col antialiased overflow-x-hidden w-full">
      ...
      <div className="lg:pl-72 flex min-h-screen min-h-dvh flex-1 flex-col transition-all duration-200 w-full">
        ...
        <main className="flex-1 w-full overflow-x-hidden px-3.5 py-4 sm:px-6 sm:py-6 lg:px-8">
          <div className="max-w-7xl mx-auto w-full min-w-0">
            <Suspense fallback={<DashboardSkeleton title={title} subtitle={subtitle} />}>
              {children}
            </Suspense>
          </div>
        </main>
