...
async function runUpdates() {
  ...
  const queries = [
    ...
    "ALTER TABLE athletes ADD COLUMN academy_name VARCHAR(255) AFTER state",
    "ALTER TABLE athletes ADD COLUMN district VARCHAR(100) AFTER state",
    "ALTER TABLE athletes ADD COLUMN joining_date DATE AFTER registration_date",
    "ALTER TABLE athletes ADD COLUMN medical_status ENUM('fit','unfit','injured','under_observation') DEFAULT 'fit' AFTER joining_date",
    "ALTER TABLE athletes ADD COLUMN current_status ENUM('active','inactive','archived','transferred') DEFAULT 'active' AFTER medical_status",
    "ALTER TABLE athletes ADD COLUMN archived_at TIMESTAMP NULL AFTER current_status",
    "ALTER TABLE athletes ADD COLUMN archived_by INT UNSIGNED AFTER archived_at",
    
    // Add columns to coaches
    "ALTER TABLE coaches ADD COLUMN coach_code VARCHAR(50) UNIQUE AFTER id",
    "ALTER TABLE coaches ADD COLUMN date_of_birth DATE AFTER coach_code",
    "ALTER TABLE coaches ADD COLUMN gender ENUM('male','female','other') AFTER date_of_birth",
    "ALTER TABLE coaches ADD COLUMN address TEXT AFTER gender",
    "ALTER TABLE coaches ADD COLUMN joining_date DATE AFTER address",
    "ALTER TABLE coaches ADD COLUMN current_status ENUM('active','inactive','archived') DEFAULT 'active' AFTER is_active",
    
    // Add columns to selectors
    "ALTER TABLE selectors ADD COLUMN selector_code VARCHAR(50) UNIQUE AFTER id",
    "ALTER TABLE selectors ADD COLUMN sport_expertise VARCHAR(255) AFTER organization",
    "ALTER TABLE selectors ADD COLUMN years_experience INT DEFAULT 0 AFTER sport_expertise",
    
    // Insert seeds
    `INSERT IGNORE INTO sports (name, description) VALUES
      ('Football', 'Association football / soccer'),
      ('Cricket', 'Cricket - bat and ball sport'),
      ('Athletics', 'Track and field athletics'),
      ('Swimming', 'Competitive swimming'),
      ('Badminton', 'Racket sport'),
      ('Basketball', 'Team basket ball')`,
